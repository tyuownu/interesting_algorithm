...
void quickSort(IntWithIndex *p, int begin, int end)
{
  if(begin < end)
  {
    int left = begin, right = end;
    IntWithIndex temp = p[begin];

    while(left < right)
    {
      while((left < right) && (p[right].data > temp.data)) --right;
      if(left < right) p[left++] = p[right];
      while((left < right) && (p[left].data < temp.data)) ++left;
      if(left < right) p[right--] = p[left];
    }
    p[left] = temp;
    quickSort(p, begin, left - 1);
    quickSort(p, left+1, end);
  }
}

int binarySearch(IntWithIndex *p, int size, int target)
{
  int left = 0, right = size-1;
  while(left <= right)
  {
    int mid = (left + right)/2;
    if(p[mid].data == target) return p[mid].index;
    else if(p[mid].data < target) left = mid+1;
    else right = mid-1;
  }
  return -1;
}
...
{
  IntWithIndex *p;
  p = malloc(n * sizeof(IntWithIndex));
...
//    printf("index: %d value %d\n", p[i].index, p[i].data);
  ...
  for(int i = 0; i < n; ++i)
  {
    int other_number = target - p[i].data;
    int other_index = -1;
    other_index = binarySearch(p, n, other_number);
    if(other_index >= 0)
    {
      if(p[i].index > other_index)
      {
        index[0] = other_index;
        index[1] = p[i].index;
        //printf("%d -- %d, and %d -- %d\n", p[i].index, p[i].data, other_index, numbers[ other_index ]);
        break;
      }
      else if(p[i].index < other_index)
      {
        index[0] = p[i].index;
        index[1] = other_index;
        //printf("%d -- %d, and %d -- %d\n", p[i].index, p[i].data, other_index, numbers[ other_index ]);
